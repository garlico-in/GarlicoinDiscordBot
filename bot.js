...
client.on("ready", () => {
    ...
    client.user.setActivity(`Garlicoin | ${config.prefix}help`, { type: 'WATCHING' });

    serverChannels = client.channels
...
client.on("messageCreate", message => {
    ...
    if (command === "help") {
        ...
        let next_payout;
        try { next_payout = new Date(jsonStats.body?.primary.payments.next).toISOString(); } catch { };
        let last_payout;
        try { last_payout = new Date(jsonStats.body?.primary.payments.last).toISOString(); } catch { };
        let msg = '';
        msg += '```ml\n';
        msg += 'Info\n'
        ...
        msg += `  Next Payout        : ${next_payout}\n`;
        msg += `  Last Payout        : ${last_payout}\n`;
        msg += 'Servers\n';
...
function broadcast(message) {
    ...
    //    for(channel in client.channels)(function (chan) {
    //        if (chan.type === "text") {
